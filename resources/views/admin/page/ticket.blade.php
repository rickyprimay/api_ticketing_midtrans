...
@section('content')
            ...
            <table id="ticketTable"
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Stock Tiket</th>
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <form action="{{ route('admin.ticket.store') }}" method="POST" class="p-4 md:p-5">
                            ...
                            <div class="grid gap-4 mb-4 grid-cols-2">
                                ...
                                <div class="col-span-2">
                                    <label for="unlimited_stock" class="flex items-center space-x-2">
                                        <input type="checkbox" name="unlimited_stock" id="unlimited_stock"
                                            class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800">
                                        <span class="text-sm font-medium text-gray-900 dark:text-gray-300">Apakah ticket
                                            tidak terbatas?</span>
                                    </label>
                                </div>
                            </div>
        ...
        <div id="edit-modal-{{ $ticket->ticket_id }}" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <form action="{{ route('admin.ticket.update', $ticket->ticket_id) }}" method="POST"
                        class="p-4 md:p-5">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="ticket_type"
                                    class="block text-sm font-medium text-gray-900 dark:text-white mb-2">Tipe Tiket</label>
                                <input type="text" name="ticket_type" id="ticket_type"
                                    value="{{ $ticket->ticket_type }}"
                                    class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:border-primary-600 dark:bg-gray-700 dark:text-white dark:border-gray-500 dark:focus:border-primary-500"
                                    required>
                            </div>
                            <div>
                                <label for="event_id"
                                    class="block text-sm font-medium text-gray-900 dark:text-white mb-2">Event Asal
                                    Tiket</label>
                                <select name="event_id" id="event_id"
                                    class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:border-primary-600 dark:bg-gray-700 dark:text-white dark:border-gray-500 dark:focus:border-primary-500"
                                    required>
                                    @foreach ($event as $eventsa)
                                        <option value="{{ $eventsa->event_id }}"
                                            {{ $ticket->event_id == $eventsa->event_id ? 'selected' : '' }}>
                            ...
                            <div>
                                <label for="price"
                                    class="block text-sm font-medium text-gray-900 dark:text-white mb-2">Harga
                                    Tiket</label>
                            ...
                            <div>
                                <label for="stock" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Stock Tiket</label>
                                <input type="number" name="stock" id="stock" value="{{ $ticket->stock }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    placeholder="">
                            </div>
                            <div class="pb-4">
                                <label for="unlimited_stock" class="flex items-center space-x-2">
                                    <input type="checkbox" name="unlimited_stock" id="unlimited_stock"
                                        class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
                                        {{ $ticket->stock === null ? 'checked' : '' }}>
                                    <span class="text-sm font-medium text-gray-900 dark:text-gray-300">Apakah ticket tidak terbatas?</span>
                                </label>
                            </div>
                        </div>
                        <button type="submit"
                            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Edit</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Menangani semua modal edit
            document.querySelectorAll('[id^="edit-modal-"]').forEach(modal => {
                const checkbox = modal.querySelector('#unlimited_stock');
                const stockInput = modal.querySelector('#stock');

                function toggleStockInput() {
                    if (checkbox.checked) {
                        stockInput.style.display = 'none';
                        stockInput.value = ''; // Kosongkan input stock jika tidak terbatas
                    } else {
